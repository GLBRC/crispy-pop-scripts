...
parameters:
  ...
  -m, --mismatch      Number of offsite target mismatches allowed only 0 or 1 allowed
...
pamOptions = { 'NGG':'3', 'NNGRRT':'3', 'TTTV':'3', 'NNNNGATT':'3', 'NNAGAAW':'3', 'TTTN': '3', 'NCC': '3'}
##### PAM sequence lengths, used to trim custom sgRNA sequences used to search for off site targets
pamLen     = { 'NGG': 3,  'NNGRRT': 6,  'TTTV': 4,  'NNNNGATT':'8', 'NNAGAAW':'7', 'TTTN': 4, 'NCC': 3}
...
class Crispy( object ):
    ...
    def parseScorer(self, numMisMatch, altRef=''):
        """
        ...
        with open(path + 'tmp/cas-in-' + self.runID + '.txt','w') as casfile:
            ...
            for k in sgRNA.keys():
                casfile.write("%s %s\n" %(k,numMisMatch))                              # write sgRNA guide rna's to file
    ...
    def prepSingleOffSite(self, seq, numMisMatch, altRef='' ):
        """
        ...
        with open(path + 'tmp/cas-in-' + self.runID + '.txt','w') as casfile:
            ...
            casfile.write("%s\n" %(pattern))                                   # match pattern
            casfile.write("%s %s\n" %(seq, numMisMatch))                                # write sgRNA guide rna's to file
...
def main():
    ...
    cmdparser.add_argument('-m', '--mismatch', action='store', dest="MISMATCH",
                            help='Offsite target mismatch value [0,1]', metavar='')
    ...
    if cmdResults['INFO']:
        ...
        print( "\n\t   -m     Number of offsite target mismatches allowed 0 or 1")
    ...
    # check offsite target mismatch value only 0 and 1 are allowed
    if cmdResults['MISMATCH']:
        mismatch = cmdResults['MISMATCH']
        if mismatch not in [ '0', '1']:
            print("\n\tOffsite mismatch value must be 0 or 1")
            logging.error("Offiste mismatch value not 0 or 1, but is %s" %(mismatch))
            cmdparser.print_help()
            sys.exit(1)
    else:  # default mismatch set to 0
        mismatch = '0'
    ...
    if cmdResults['SGRNA']:
        ...
        if cmdResults['REF']:
            ...
            guide.prepSingleOffSite(sgRNA, mismatch, altRef)
            guide.findOffSiteTargets()
            guide.singleWriteResults(altRef)
        else:
            guide.prepSingleOffSite(sgRNA, mismatch)
            guide.findOffSiteTargets()
    ...
    #user inputs sequence to search for CRISPR guide RNA's
        ...
        if cmdResults['REF'] and cmdResults['REF'].upper() != 'NONE':
            altRef = guide.setAltRef(cmdResults['REF'])
            sgRnaDict      = guide.parseScorer(mismatch,altRef)                         # dictionary of sgRNA results, key= guide sequence
            offSiteTargets = guide.findOffSiteTargets()
        # assume no reference, no offsite checking
        else:
            sgRnaDict      = guide.parseScorer(mismatch)                               # dictionary of sgRNA results, key= guide sequence
